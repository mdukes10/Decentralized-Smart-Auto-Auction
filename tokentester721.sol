...
contract MyCar is ERC721{
    ...
    Car[] public cars; // First Item has Index 0
    ...
    function mintCar(address account, string memory vin, string memory year, string memory make, string memory model, string memory state, uint miles, uint accidents, uint initial_value) public {
        require(owner == msg.sender); // Only the Owner can create Items
        uint256 carId = cars.length; // Unique card ID
        cars.push(Car(vin, year, make, model, state, miles, accidents, initial_value));
        _mint(account, carId); // Mint a new card
    }
